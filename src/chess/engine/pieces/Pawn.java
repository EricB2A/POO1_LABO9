...
public class Pawn extends Piece {
    ...
    public List<Move> getMoves(int x, int y) {
        ...
        // avance un coup
        if (Move.inBound(x, y + deltaPlayer, board.getDimension()) && board.isCellFree(x, y + deltaPlayer)) {
        ...
        // attaque droite
        if (Move.inBound(x + 1, y + deltaPlayer, board.getDimension()) && !board.isCellFree(x + 1, y + deltaPlayer) && ((Piece) board.getPiece(x + 1, y + deltaPlayer)).getOwner() != getOwner()) {
            if (canBePromoted(y + deltaPlayer)) {
        ...
        if (lastMove != null && lastMove.getSpecialMove() == SpecialMove.PAWN_FAST_MOVE) {
            if (lastMove.getToY() == y) {
                ...
                if (Move.inBound(x + 1, y, board.getDimension()) && lastMove.getToX() == x + 1
                        && board.isCellFree(x + 1, y + deltaPlayer)) {

                    moves.add(new Move(x, y, x + 1, y + deltaPlayer, SpecialMove.PAWN_EN_PASSANT));
                }
                // diagonale gauche
                else if (Move.inBound(x - 1, y, board.getDimension()) && lastMove.getToX() == x - 1
                        && board.isCellFree(x - 1, y + deltaPlayer)) {
                    moves.add(new Move(x, y, x - 1, y + deltaPlayer, SpecialMove.PAWN_EN_PASSANT));
                }
            }
